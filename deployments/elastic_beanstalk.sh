...
IFS=' ' read -r -a ENVIRONMENTS <<< "${AWS_APP_ENVIRONMENTS:?'You need to configure AWS_APP_ENVIRONMENTS'}"

# defaults
AWS_S3_BUCKET=${AWS_S3_BUCKET:=${AWS_APP_NAME}_${CI_BRANCH}_deployment}
AWS_APP_VERSION=${AWS_APP_VERSION:=${AWS_APP_NAME}_${COMMIT_ID:0:8}_$(date -u "+%Y-%m-%d_%H%M%S")}
...
aws elasticbeanstalk create-application-version --application-name "${AWS_APP_NAME}" --version-label "${AWS_APP_VERSION:0:100}" --source-bundle S3Bucket="${AWS_S3_BUCKET}",S3Key="${AWS_APP_VERSION}.zip"

for env in "${ENVIRONMENTS[@]}"; do
  if [ "${env}" != '' ]; then
    aws elasticbeanstalk update-environment --environment-name "${env}" --version-label "${AWS_APP_VERSION:0:100}"
    # check if the deployment was successful
    eb_deployment_check "${AWS_APP_NAME}" "${env}"
  fi
done
